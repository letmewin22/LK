...
export default class FormAnimations {
  ...
  static open(form, elem) {
    ...
    const size = window.innerWidth > window.innerHeight ? window.innerWidth * 2 : window.innerHeight * 2
    const tl = new TimelineMax()
    tl
      .to(elem.querySelector('.form-bg'), 1, { width: size, height: size, left: 0, ease: Power3.easeInOut })
      .to(elem.querySelector('.form-bg'), 1, { backgroundColor: '#f1f1f1', ease: Power1.easeInOut }, 0)
  ...
  static close(elems, callback) {
    ...
    tl
      ...
      .to(document.querySelectorAll('.form-bg'), 1, { width: 0, height: 0, left: '50%', ease: Expo.easeOut }, 0.8)
      .to(document.querySelectorAll('.form-bg'), 1, { backgroundColor: '#9c27b0', ease: Expo.easeOut }, 0.8)
